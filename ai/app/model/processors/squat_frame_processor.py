...
import cv2
import numpy as np
import app.model.features as features

from app.util.landmark_utils import find_angle, get_landmark_features
from app.util.drawing_utils import draw_text, draw_dotted_line


def _show_feedback(frame, c_frame, dict_maps, lower_hips_disp):
    if lower_hips_disp:
        draw_text(
            frame,
            'LOWER YOUR HIPS',
    ...
    for idx in np.nonzero(c_frame)[0]:
        draw_text(
            frame,
            dict_maps[idx][0],
    ...
    def process(self, frame: np.array, pose):
        ...
        if key_points.pose_landmarks:
            ...
            # Camera is aligned properly.
                ...
                frame = _show_feedback(frame, self.state_tracker['COUNT_FRAMES'], self.FEEDBACK_ID_MAP,
